interface MRAID {
    ...
    /**
     * The expand method will cause an existing Web View (for one-part creatives) or a new Web View (for two-part creatives) to open at the highest z-order in the view hierarchy.
     * When the expand method is called without the URL parameter, the current web view will be reused, simplifying reporting and ad creation. 
...
class ExpandProperties {
    ...
    constructor(width: number, height: number, useCustomClose: boolean, isModal: boolean) {
        this.width = width;
        this.height = height;
